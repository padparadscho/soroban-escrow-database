...
1.  **Fork** the `soroban-escrow-database` repository to a personal GitHub account.
2.  **Clone** the forked repository to a local machine:

```bash
git clone https://github.com/<your-username>/soroban-escrow-database.git
...
Bugs are reported and new features are requested by opening an [**Issue**](https://github.com/padparadscho/soroban-escrow-database/issues) using the appropriate template.
...
- **Small Features & Bug Fixes** A [**Pull Request**](https://github.com/padparadscho/soroban-escrow-database/pulls) can be submitted directly.
